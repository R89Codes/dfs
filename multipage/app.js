...
(() => {
  ...
  function renderGraph() {
    ...
    state.graph.nodes.forEach((node) => {
      ...
      const circle = svgEl('circle', { cx: pos.x, cy: pos.y, r: NODE_RADIUS, 'stroke-width': 1.5 });
      const label = svgEl('text', { x: pos.x, y: pos.y + 4, 'text-anchor': 'middle', 'font-size': 12, 'font-weight': 700, fill: '#fff2e7' });
      label.textContent = node;
      const stamp = svgEl('text', { x: pos.x, y: pos.y + 40, 'text-anchor': 'middle', 'font-size': 11, fill: '#d9bfa9' });
      nodeLayer.appendChild(halo);
      nodeLayer.appendChild(circle);
      nodeLayer.appendChild(label);
      nodeLayer.appendChild(stamp);
      nodeEls.set(node, { halo, circle, stamp });
    });
  }
  ...
  function edgeGeom(from, to) {
    ...
    const sx = a.x + ux * NODE_RADIUS + px * curve * 0.25;
    const sy = a.y + uy * NODE_RADIUS + py * curve * 0.25;
    const ex = b.x - ux * NODE_RADIUS + px * curve * 0.25;
    const ey = b.y - uy * NODE_RADIUS + py * curve * 0.25;
